...
from utils import load_conjugations
...
def main():
    correct_conjugations = load_conjugations("data/conjugaciones_correctas.txt")
    incorrect_conjugations = load_conjugations("data/conjugaciones_incorrectas.txt")

    population = [create_random_afd(num_states=5, alphabet=ALPHABET) for _ in range(POPULATION_SIZE)]
